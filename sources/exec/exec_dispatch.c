...
/*   Updated: 2025/11/14 22:06:46 by adeimlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static
int stt_dispatch(t_argv *argv, t_env *env)
{
	...
	if (str[0] == 'e')
	{
		...
		if (str[1] == 'x' && str[2] == 'p' && str[3] == 'o' && str[4] == 'r'
			&& str[5] == 't' && stt_end(str[6]))
			_exit (msh_export(argc, argv, envp));
		if (str[1] == 'n' && str[2] == 'v' && stt_end(str[3]))
			_exit (msh_env(argc, argv, envp));
		if (str[1] == 'c' && str[2] == 'h' && str[3] == 'o' && stt_end(str[4]))
			_exit (msh_echo(argc, argv, envp));
		return (1);
	}
	if (str[0] == 'c' && str[1] == 'd' && stt_end(str[2]))
		_exit (msh_cd(argc, argv, envp));
	if (str[0] == 'p' && str[1] == 'w' && str[2] == 'd' && stt_end(str[3]))
		_exit (msh_pwd(argc, argv, envp));
	if (str[0] == 'u' && str[1] == 'n' && str[2] == 's' && str[3] == 'e'
		&& str[4] == 't' && stt_end(str[5]))
		_exit (msh_unset(argc, argv, envp));
	return (1);
}